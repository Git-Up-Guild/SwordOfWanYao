...
namespace UnityEngine.Experimental.Rendering.Universal
{
    internal class Renderer2D : ScriptableRenderer
    {
            ...
            out RenderTargetHandle depthTargetHandle)
        {
            ...
            {
                if(IsRender2DefauktTarget()==false)
                {
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            for (int i = 0; i < rendererFeatures.Count; ++i)
            {
                if (rendererFeatures[i].isActive)
                {
                    ...
                    //假如是覆盖模式，设置一下渲染缓冲
                    if(rendererFeatures[i].IsoverrideCameraTarget())
                    {
                        rendererFeatures[i].ConfigureTarget(colorTargetHandle.Identifier(), depthTargetHandle.Identifier());
                    }
                }
        ...
        protected override bool Is2DRender()
        {
            return true;
        }
    }
