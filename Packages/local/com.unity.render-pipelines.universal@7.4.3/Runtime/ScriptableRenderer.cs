...
namespace UnityEngine.Rendering.Universal
{
    ...
    {
        //是否启用简单渲染
        public static bool s_enableSimpleRender = true;

        //开启渲染到默认缓冲区标识
        public static bool s_render2DefaultTarget = true;

        //当前帧是否有开后处理
        public static bool s_cameraDisableProcess = true;

        //是否需要深度图
        public static bool s_requiresDepthTexture = false;


#if UNITY_EDITOR 
        //当前帧是否有开后处理
        public static bool s_enableSimpleSkyBox = false;
#else
  public static bool s_enableSimpleSkyBox = true;
#endif

        //是否一帧开始

        public static bool s_forceSetRenderTarget = false;
        //容错帧
        public static int  s_adjustFrame = 0;


        static RenderTargetIdentifier m_LastCameraColorTarget;
        static RenderTargetIdentifier m_LastCameraDepthTarget;


        public static bool IsRender2DefauktTarget()
        {
            return s_render2DefaultTarget&& s_cameraDisableProcess;
        }
        ...
        //是否开始简单渲染
        protected bool m_bSimpleRender = false;
        ...
        public void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if(HadExcutePass(RenderPassBlock.BeforeRendering, blockRanges))
            {
...
#region deprecated
        ...
        protected virtual bool Is2DRender()
        {
            return false;
        }
        ...
        bool HadExcutePass(int blockIndex, NativeArray<int> blockRanges)
        {
            int currIndex = blockRanges[blockIndex];
        ...
        void ExecuteRenderPass(ScriptableRenderContext context, ScriptableRenderPass renderPass, ref RenderingData renderingData, int eyeIndex)
        {
            ...
            else
            {
                ...
                // Only setup render target if current render pass attachments are different from the active ones
                if (s_forceSetRenderTarget||passColorAttachment != m_ActiveColorAttachments[0] || passDepthAttachment != m_ActiveDepthAttachment || finalClearFlag != ClearFlag.None)
                {

                    //之前是普通目标渲染,&第一次
                    bool bForeceChange = s_forceSetRenderTarget;//&&((m_LastCameraColorTarget != BuiltinRenderTextureType.CameraTarget)|| (s_adjustFrame >= 60)|| cameraData.postProcessEnabled);
                    ...
                    //判断目标是否改变
                    bool bSameTarget = (passColorAttachment == m_LastCameraColorTarget && passDepthAttachment == m_LastCameraDepthTarget && !bForeceChange);//

                    if (s_forceSetRenderTarget)
                    {
                    ...
                    s_forceSetRenderTarget = false;


                    m_LastCameraColorTarget = passColorAttachment;
                    m_LastCameraDepthTarget = passDepthAttachment;

                    //m_LastCameraColorTarget = m_CameraColorTarget;
                   // m_LastCameraDepthTarget = m_CameraDepthTarget;

                    SetRenderTarget(cmd, passColorAttachment, passDepthAttachment, finalClearFlag, finalClearColor, bSameTarget);
                }
        ...
        internal static void SetRenderTarget(CommandBuffer cmd, RenderTargetIdentifier colorAttachment, RenderTargetIdentifier depthAttachment, ClearFlag clearFlag, Color clearColor, bool bSameTarget = false)
        {
            ...
            //直接清理深度
            if (bSameTarget)// && colorLoadAction == RenderBufferLoadAction.DontCare&& RenderBufferLoadAction.DontCare== depthLoadAction)
            {
                //保持所有
                if (colorLoadAction == RenderBufferLoadAction.Load && RenderBufferLoadAction.Load == depthLoadAction)
                {
                    return;
                }

                //丢弃所有
                if (colorLoadAction == RenderBufferLoadAction.DontCare && RenderBufferLoadAction.DontCare == depthLoadAction)//
                {
                    CoreUtils.ClearRenderTarget(cmd, ClearFlag.Depth, clearColor);
                    return;
                }
        ...
        //是否支持简单渲染
        public virtual bool CanSupportSimpeleRender()
        {
            return false;
        }

         public void EnableSimpleRender(bool enable)
        {
            m_bSimpleRender = enable;
        }

        public bool IsSimpleRender()
        {
            return m_bSimpleRender;
        }
