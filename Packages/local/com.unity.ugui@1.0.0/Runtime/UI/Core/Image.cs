...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        [NonSerialized]
        private Sprite m_lastDidSprite = null;
        private Color  m_lastDidColor = Color.blue;
        //private bool   m_didColorOnly = false; 
        ...
        {
            if(m_didColorOnly&&!m_MaterialDirty)
            {
            }
        ...
        */
        /*
        protected override void OnChangeColor()
        {
            //m_didColorOnly = true;
            m_lastDidColor = color;
            base.OnChangeColor();
        }
        */
        protected override void OnDidApplyAnimationProperties()
        {
            ...
            //颜色和材质已经有单独控制
            //动画改图片,走原来的途径
            if(m_lastDidSprite!=activeSprite)
            {
                m_lastDidSprite = activeSprite;
                //SetAllDirty();
                ...
                SetVerticesDirty();
                SetRaycastDirty();
                //m_didColorOnly = false;


            }
            else if (SetPropertyUtility.SetColor(ref m_lastDidColor, color))
            {
                SetVerticesDirty();
            }
            /*
            ...
            {
                Debug.LogError("跳过 OnDidApplyAnimationProperties");
            }
            */
  ...
  public override Material materialForRendering
        {
            get
            {
                ...
                if (m_ShouldRecalculateStencil|| m_StencilValue > 0 || isMaskingGraphic)
                {
                    //currentMat = GetModifiedMaterial(currentMat);
                    return base.materialForRendering;
                }else
                {
                   // Debug.LogError("缺省材质: name=" + this.name);
                    return material;
                }
