...
namespace UnityEngine.UI
{
    ...
    public class GraphicRegistry
    {
        ...
        //
        static private Stack<IndexedSet<Graphic>> m_indexSetPool = new Stack<IndexedSet<Graphic>>(32);
        ...
        //分配一个对象
        private static IndexedSet<Graphic> AlocGrahicIndex()
        {
            if(m_indexSetPool.Count>0)
            {
                IndexedSet<Graphic> indSet = m_indexSetPool.Pop();
                return indSet;
            }

            return new IndexedSet<Graphic>();
        }

        //回收一个对象
        private static void RecycleGrahicIndex(IndexedSet<Graphic> indSet)
        {
            if(null== indSet)
            {
                return;
