...
namespace UnityEngine.UI
{
    ...
    public class CanvasUpdateRegistry
    {
        ...
        //layout 对象
        public static Dictionary<RectTransform,Graphic> LayoutDirty = new Dictionary<RectTransform, Graphic>();
        ...
        private bool ObjectValidForUpdate(ICanvasElement element,bool checkActive = false)
        {
            ...
            {
                return element.transform.gameObject.activeInHierarchy;
            }
        ...
        private void CleanInvalidItems()
        {
            ...
            //if(layoutRebuildQueueCount>0)
            //Debug.LogError("layoutRebuildQueueCount=" + layoutRebuildQueueCount);
            ...
            //if (graphicRebuildQueueCount > 0)
            //    Debug.LogError("graphicRebuildQueueCount="+ graphicRebuildQueueCount);
        ...
        private void PerformUpdate()
        {
            ...
            {
                LayoutRebuilder.ClearRebuildItems();
            ...
            for (var i = (int)CanvasUpdate.PreRender; i < (int)CanvasUpdate.MaxUpdateValue; i++)
            {
                ...
                for (var k = 0; k < m_GraphicRebuildQueue.Count; k++)
                {
...
#endif
                    {
                        ...
                        if (ObjectValidForUpdate(element,true))
        ...
        public static void AddLayoutRectTransform(RectTransform rt, Graphic grp)
        {
            if(null== rt || LayoutDirty.ContainsKey(rt))
            {
                //Debug.LogError("AddLayoutRectTransform 重复 rebuild rt="+ rt.name);
                return;
            }

            LayoutDirty.Add(rt, grp);
        }

        public static void LayoutAllRectTransform()
        {
            Graphic gp = null;
            bool bHadLayout = false;
            foreach (var rect in LayoutDirty.Keys)
            {
                if(null!= rect)
                {
                    bHadLayout = LayoutRebuilder.RealMarkLayoutForRebuild(rect);
                    gp = LayoutDirty[rect];
                    if(null!=gp)
                    {
                        gp.SetLayoutFlag(bHadLayout);
                    }
