		...
		SubShader
		{
			...
			Pass {
				...
				fixed4 frag ( v2f i , half ase_vface : VFACE ) : SV_Target
				{
					...
					float2 appendResult186 = (float2(_SpeedMainTexUVNoiseZW.x * _UVSpeedControl, _SpeedMainTexUVNoiseZW.y * _UVSpeedControl));
					float2 uv_MainTex = i.texcoord.xy * _MainTex_ST.xy + _MainTex_ST.zw;
					float2 uv_Mask = i.texcoord.xy * _Mask_ST.xy + _Mask_ST.zw;
					float2 appendResult177 = (float2(_SpeedMainTexUVNoiseZW.z * _UVSpeedControl, _SpeedMainTexUVNoiseZW.w * _UVSpeedControl));
