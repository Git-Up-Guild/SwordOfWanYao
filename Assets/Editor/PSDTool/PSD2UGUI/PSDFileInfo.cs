    ...
    public class PSDFileInfo
    {
        ...
        public PSDFileInfo(PsdFile psd)
        {
            ...
            for (int i = psd.Layers.Count - 1; i >= 0; i--)
            {
                ...
                layer.Name = layer.Name.Trim(); //去点前后空格

                if (layer.Name == UGUI2PSD.UGUI2PSDTool.UserLayerName)
                {
                    UserLayerIndex = i; //记录用户层信息索引
                    layer.Visible = false;  //强行隐藏用户层
                    //continue;
                }
        ...
        public PSDLayerGroupInfo GetRootGroupByLayerIndex(int layerIndex)
        {
            List<PSDLayerGroupInfo> candidates = new List<PSDLayerGroupInfo>();
            // Might be a nested layer group
            foreach (var layerGroupInfo in LayerGroups)
            ...
            return candidates.OrderBy(info => info.end - info.start).LastOrDefault();
        }

        public string GetRootGroupNameByLayerIndex(int layerIndex)
        {
            PSDLayerGroupInfo pSDLayerGroupInfo = GetRootGroupByLayerIndex(layerIndex);
            if (pSDLayerGroupInfo != null)
            {
                return pSDLayerGroupInfo.name;
            }
            return null;
        }

        public PSDLayerGroupInfo GetGroupInfoByName(string groupName)
        {
            foreach (var layerGroupInfo in LayerGroups)
            {
                if (layerGroupInfo.name == groupName)
