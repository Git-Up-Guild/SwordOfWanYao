...
namespace subjectnerdagreement.psdexport
{
    ...
    public class PSDExportSettings
    {
        ...
        public const string CommonExportDirName = "Other";

        //public const string LocalCommonGroupName = "LocalCommon";   //本psd的公共资源
        //public const string ErrorNameResPostfix = "-error";
        ...
        public class LayerSetting
        {
            ...
            //导入信息
            public ImportResInfo importInfo;
        }
        ...
        public bool IsCommonPSD { get; set; }
        ...
        /// 是否开启伽马校正
        /// </summary>
        public bool IsNeedAlphaCorrect
        {
            get
            {
                //return PlayerSettings.colorSpace == ColorSpace.Linear;  //线性模式下才需要伽马校正
                return false;
            }
        }


        public static bool IsCloseSRGB { get; set; }

        public static bool IsUseTextMeshPro { get; set; }
        ...
        public PSDExportSettings(string path)
        {
            ...
            Psd = new PsdFile(path, Encoding.Default);
            Filename = Path.GetFileNameWithoutExtension(path);//获取文件名,包含扩展名
        ...
        private void LoadMetaData()//加载元数据
        {
            AutoReImport = false;
        ...
        public void BuildLayerImportInfo()
        {
            foreach(var layerSetting in layerSettings.Values)
            {
                var layer = Psd.Layers[layerSetting.layerIndex];
                layerSetting.importInfo = PSDSetting.Instance.GetImportResInfo(layer.Name, FullFileName);
                layerSetting.importInfo.isUpdate = layerSetting.importInfo.isUpdate || layerSetting.updateMode;
            }
        }
        ...
        public void SaveLayerMetaData()
        {
            ...
            List<LayerSetting> keys = new List<LayerSetting>(layerSettings.Values);
            for (int i = 0; i < keys.Count; i++)
            {
                SaveLayerSetting(keys[i]);
            }
        }
        ...
        {
            layerName = GetValidFileName(layerName);

            string imgPath = string.Format("{0}.png", layerName);

            if (layerName.IndexOf('/') != -1)
            {
                return string.Format("Assets/{0}/{1}", PSDSetting.Instance.importBaseDir, imgPath);
            }

            string basePath = string.Format("Assets/{0}", ExportPath);
            var layerPath = Path.Combine(basePath, imgPath);
        ...
        {
            string validName = fileName.Split('|')[0];
            ...
            foreach (char invalidChar in Path.GetInvalidFileNameChars())
            {
                if (invalidChar != '/')
                {
                    validName = validName.Replace(invalidChar, '-');
                }
            }
