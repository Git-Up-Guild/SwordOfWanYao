...
namespace WXB
{
    public delegate uint LoadSpriteFunc(string name, System.Action<Sprite,uint> callback);
    public delegate void UnloadSpriteFunc(uint handle);
    ...
    static public partial class Tools
    {
        ...
        static public Font GetFont(string name)
        {
            if (s_get_font == null)
            {
                Font f = SymbolTextSettings.GetFont(name);
                if (f == null)
                    return DefaultFont;
            }
        ...
        // 得到精灵的接口
        public static LoadSpriteFunc s_load_sprite = null;
        public static UnloadSpriteFunc s_unload_sprite = null;

        // 得到字体的接口
        public static System.Func<string, Font> s_get_font = null;

        public static uint LoadSprite(string name, System.Action<Sprite, uint> callback)
        {
            uint handle = 0;
            ...
            if (s_load_sprite != null)
            {
                handle = s_load_sprite.Invoke(name, callback);
            }
            else
            {
                callback(sp, 0);
        ...
        public static void UnloadSprite(uint resHandle)
        {
            if (resHandle > 0)
                s_unload_sprite?.Invoke(resHandle);
        }

        static public Sprite GetSprite(string name)
        {
            return SymbolTextSettings.GetSprite(name);
        }
        ...
        static public Cartoon GetCartoon(string name)
        {
            if (s_get_cartoon == null)
                return SymbolTextSettings.GetCartoon(name);
        ...
        static public void GetAllCartoons(List<Cartoon> cartoons)
        {
            ...
            {
                SymbolTextSettings.GetCartoons(cartoons);
                return;
            }
